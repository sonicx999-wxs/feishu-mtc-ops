...
name: feishu-mtc-ops
version: 2.3.1
description: "飞书 MTC 模式操作技能：在 Trae MTC 模式下进行飞书文档操作（创建、获取、更新、搜索、媒体管理）、聊天操作（发送消息、获取聊天记录、创建群聊）、云空间操作（上传、下载、导出）、电子表格操作、多维表格操作、知识库操作等。默认以用户身份操作，支持应用身份创建文档后自动添加权限。包含首选方案和备用方案的完整执行流程。支持文件夹管理和默认目录配置。支持自然语言操作引导和智能错误处理。支持新版env格式和旧版args格式的MCP凭证配置。"
metadata:
  requires:
    bins: ["lark-cli"]
    mcps: ["mcp_lark-mcp"]
...
> ⚠️ **注意**：飞书 API 没有直接的群聊搜索接口，需要通过获取列表后本地匹配实现。
...
**步骤 2**：在返回结果中按名称模糊匹配
- 遍历 `items` 数组
- 匹配 `name` 字段包含查询关键词的群聊
- 返回匹配结果给用户选择

**匹配逻辑示例**：
```javascript
// 本地模糊匹配逻辑
const matched = items.filter(chat => {
...
**方法三**：从群成员列表获取（见 [3.6 获取群成员列表](#36-获取群成员列表)）
